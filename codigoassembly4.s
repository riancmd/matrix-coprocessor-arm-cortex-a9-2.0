...
result_buffer:
    .space 16
O_RDWR_O_SYNC:       
    .word 0x10002      @ O_RDWR (2) | O_SYNC (10000) em hexadecimal
FPGA_BRIDGE_PHYS:    
    .word 0xFF200   @ Endereço físico da FPGA
...
_start:
    ...
    mov r1, #PAGE_SIZE     @ Tamanho do mapeamento (1 página)
    mov r2, #PROT_READ | PROT_WRITE
    mov r3, #MAP_SHARED
    mov r4, r10            @ File descriptor do /dev/mem
    ldr r5, =FPGA_BRIDGE_PHYS @ Endereço físico da FPGA
    mov r7, #SYSCALL_MMAP
    swi 0
    cmp r0, #MAP_FAILED
    beq mmap_fail
...
op_soma:
    ...
    @ Aguarda o resultado (verifica bit ready)
wait_ready:
...
int_to_ascii:
    ...
    mov r4, r1                  @ Salva início do buffer
...
convert_digits:
    ...
    mov r0, r4                  @ Início do buffer
    add r1, r4, r3              @ Fim do buffer (r3 = tamanho-1)
    sub r1, r1, #1

reverse_loop:
    cmp r0, r1                  @ CORREÇÃO: Comparação válida (2 operandos)
    bge conversion_done
    ldrb r2, [r0]               @ Troca bytes
    ldrb r5, [r1]
    strb r5, [r0]
    strb r2, [r1]
    add r0, r0, #1              @ Avança início
    sub r1, r1, #1              @ Recua fim
    b reverse_loop
